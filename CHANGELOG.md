...
- **退出码只剩两个出口**：`0` = 继续安装，`1602` = `ERROR_INSTALL_USEREXIT`（MSI 据此干净地报
  "用户已取消安装"）。旧实现返回 2（缺失）/3（去下载/取消），`Return="check"` 把任何非零码都渲染成
  那句无指引的通用错误。静默安装（`/qn`，没人能点弹窗）缺 .NET 时返回 1602，只缺 Node 时放行
...
- **三个按钮的语义按"否=不装"改**：首版草稿里【否】是"仍然继续安装"，读起来像拒绝（用户当场指出：
  "点否为什么也会安装？"）。现在【是】=装齐缺失项后继续，【否】与【取消】都**不安装**（返回 1602）。
  "仍然继续"这个选项只保留在两处**知情**场合：① 自动安装失败之后的第二问（那时用户已经看到具体
  哪一项没装上）；② 显式的高级开关 `PREREQ_FORCE_CONTINUE=1`（脚本/自带环境的人用，不是弹窗里
  一个看不懂的第三键）。
  真机实测（`PREREQ_SIMULATE_MISSING=1` 模拟双缺失 + 真实点击）：【否】→ **1602**、【取消】→
  **1602**、`PREREQ_FORCE_CONTINUE=1` → **0 且全程无弹窗**。
