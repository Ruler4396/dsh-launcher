...
- **双色小鲸鱼图标**：托盘图标固定为**白色鲸鱼**（托盘/任务栏多为深色背景，深色鲸鱼看不清）；窗口图标跟随系统深色模式——深色模式用白色鲸鱼、浅色模式用深色鲸鱼，切换主题实时更新
...
- **崩溃后服务残留无人管理**：壳拉起的服务 PID 记录到 `%LOCALAPPDATA%\dsh-launcher\service-pid-<port>.txt`；下次启动若发现该 PID 仍在监听，自动接管（"跟随窗口"关窗时一并停掉），避免进程崩溃后 node 服务永久残留占内存
