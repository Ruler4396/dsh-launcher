...
- **主题以用户的选择为主**：壳读取 dsh 设置页的主题选择（`DSH_HOME/settings.yaml` 的 `ui-theme.preference`，dark/light/system），而不是只跟随系统；用户选深色（或系统深色）时**标题栏强制深色**（DwmSetWindowAttribute 沉浸式深色标题栏）+ 白色鲸鱼图标，浅色时浅色标题栏 + 深色鲸鱼，主题切换实时生效（系统主题变化 + dsh 前端设置变化都监听）
- **托盘交互优化**：**左键单击 = 窗口置顶显示**（开着就提到最上层并聚焦，不会误关窗口）；**右键 = 只弹菜单**（不动窗口）；"显示 / 隐藏窗口"保留在右键菜单里供手动隐藏
- **托盘菜单样式 v2**：菜单项带图标（Segoe MDL2 Assets：眼睛=显示/隐藏、电源=退出）、内容自适应宽度（不再四周围空旷）、紧凑留白
- **双色小鲸鱼图标**：托盘图标固定为**白色鲸鱼**（托盘/任务栏多为深色背景，深色鲸鱼看不清）；窗口图标跟随主题（深色 → 白色鲸鱼，浅色 → 深色鲸鱼）
- **主题实时刷新（不关窗口）**：每 2 秒轮询主题设置 + 系统主题事件，深色/浅色切换后窗口与任务栏图标、标题栏**即时更新**，无需重启壳（FileSystemWatcher 对 dsh 的原子替换写入不可靠，改轮询）
...
- **深色模式下窗口/任务栏鲸鱼"消失"**：主题与图标配色逻辑写反（深色主题误用深色鲸鱼）——深色主题用白色鲸鱼、浅色用深色鲸鱼，已实测（DwmGetWindowAttribute 验证标题栏深色 + 图标配色正确）
- **主题解析误读其他配置段**：`ui-theme.preference` 的解析严格限定在 ui-theme 段内，不再误读其他段的同名字段
- **关闭窗口卡 1-2 秒**：① 不再显式 Dispose WebView2（进程退出后浏览器子进程自动清理）；② 停服务改为异步（taskkill 独立进程，壳退出不影响）——实测关窗到进程退出约 100ms
- **服务模式"常驻"被重置**：设置读取兼容旧路径（`%LOCALAPPDATA%`，旧插件写入位置）——新位置读不到时回退旧值并迁移，避免"选了常驻、壳却按跟随窗口关窗停服务"的路径错位；插件侧同时修复"每次启动无条件写回默认"（见插件 CHANGELOG）
