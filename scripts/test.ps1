...
Assert-True ($unrelLen -le 83) "【G7】CHANGELOG [Unreleased] 段 ≤ 83 行（实测 $unrelLen；定版时必须整段搬进版本标题）"
...
#   47 → 83：仓库所有者 2026-09-26 显式授权（原话"先不发版，抬高限制写入 changelog"）——本轮选择**不**用
#     定版搬段来清空 [Unreleased]，所以按上面那条沿革里既有的"同等口径显式授权抬数"这条路走。
#     入账的是静态检查与依赖漏洞监测落地一条（36 行；上一轮因顶格被本线自己撤回，现按授权补回）。
#     仍钉当下实测值、余量 +0；下一次定版必须整段搬走并当场收回。
#     同时核对过：09-25 以来 CHANGELOG.md 的全部删除行都是本线自己撤回的那条，**没有**其他线的条目
#     因顶格被挤掉 ⇒ 本次除补回自己这条外无别的入账。
