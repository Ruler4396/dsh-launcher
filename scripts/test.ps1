...
Assert-True ($unrelLen -le 454) "【G7】CHANGELOG [Unreleased] 段 ≤ 454 行（实测 $unrelLen；定版时必须整段搬进版本标题）"
# 上限 315 → 400 是**用户 2026-09-19 明确授权**的例外，不是本闸被绕过：当时该段已被并行会话的
# DPI 批次填到 315/315，任何一条新记录都会红（含本轮防臃肿整改自己的记录）。授权范围=这一次、
# 这个数值；再往上抬仍需同等授权。段长仍只降不升：低于上限时应收紧到当下实测值。
# 400 → 432：**用户 2026-09-20 明确授权**的第二次例外（同日 CI 分层整改要写记录，段已 400/400 满格）。
# 432 → 454：同一条口令的延续（用户当日对"放宽上限记录本轮"的要求），不是本闸第三次被单独抬高——
#   同日第二问的测试内容审计（删 27 条假断言、重写 3 处）要落记录，压缩到不损事实的下限后仍超 22 行。
# 两次都严格照"钉当下实测值、余量 +0"执行；下一轮段长若下降，按规则三同步改小。
