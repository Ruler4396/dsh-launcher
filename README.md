...
[DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)（`dsh`）官方提供的是命令行 + 网页版：每次都要敲 `npx @deepseek-ai/dsh web`、在浏览器里开一个标签页。dsh-launcher 是一个 **Windows 原生壳**：双击即用、可开机自启、独立小窗口，并把服务生命周期与出错诊断管起来——让 dsh 用起来像普通桌面软件。

它不是官方产品，也不内置 dsh；安装包只有 **~1.4MB**，运行依赖按需补齐（Node.js 缺失时自动下载便携版到用户目录，不改系统环境）。
...
1. 下载 `dsh-launcher-windows-<版本>.zip`（如 `dsh-launcher-windows-0.3.1.zip`），解压到任意文件夹
2. 双击 `DshWeb.exe` 即可
...
**克制**。dsh-launcher 只做一件事：还原你用 dsh 的原版体验（同一个 Web UI、同一份数据、同样的插件），把"启动、生命周期、出错诊断"这些桌面化琐事管起来。除此之外**不会额外加东西**——没有内置文件管理器、没有内置终端、没有自定义前端改造。它是个壳，不是个平台；装包 ~1.4MB 就是这种克制的结果。

## 非目标

- **不是官方产品**：与 DeepSeek / DeepSeek AI 无关，见文末免责声明。
- **不是桌面 IDE**：不做文件面板、内置终端等功能膨胀——它只是"启动 dsh + 管好生命周期 + 可诊断"的壳。

## 能力一览

- **统一日志**：壳决策 + dsh 服务输出合并为 `~/.dsh\dsh-launcher\dsh.log` 一份文件（`DSH_LOG_LEVEL` 可设 `INFO`/`WARN`/`ERROR` 过滤），自动轮转（>30MB 或 >3 天）。
- **错误码 `[E####]`**：任何报错弹窗都带错误码（如 `E2002` 服务启动超时、`E2004` 服务不可用、`E2011` 插件缺失配置降级），与日志、诊断导出共用一套码，可直接粘贴到 Issue。
- **便携 Node 自动补齐**：缺 Node.js 时确认一下，自动下载 LTS 便携版到 `%LOCALAPPDATA%\dsh-launcher\env\node\`（不改系统环境）。
- **托盘按需显示**：默认隐藏；仅在**托盘驻留**模式下常驻显示（关窗藏到托盘需唤窗入口），或有更新待通知时临时出现，不打扰用户。
...
**Q：支持哪些 dsh 版本？**
不锁定 dsh 版本，跟随你本地安装的版本（或 npx 拉取的最新版）。壳只调用 `dsh web` 的稳定 CLI 接口，dsh 升级一般无需更新启动器；若官方变更启动参数/端口，用 `DSH_WEB_URL` 环境变量即可免重建（细节见 [docs/DETAILS.md](docs/DETAILS.md) 版本兼容性）。
